...
<head>
    ...
    <style>
        ...
        .header { background-color: #1e293b; color: white; padding: 40px 20px; text-align: center; border-radius: 20px 20px 0 0; }
        ...
        .btn { display: inline-block; background-color: #ea580c; color: white; padding: 15px 30px; text-decoration: none; border-radius: 12px; font-weight: bold; margin-top: 20px; text-align: center; }
    </style>
</head>
    <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background-color: #1e293b; color: white; padding: 40px 20px; text-align: center; border-radius: 20px 20px 0 0;">
            ...
            <div style="background-color: #ffffff; padding: 40px 30px; border: 1px solid #f3f4f6; border-radius: 0 0 20px 20px;">
                <p>Hello Admin,</p>
                <p>A new order has been placed on the platform. Please find the details below:</p>
    
                <div style="font-size: 14px; font-weight: bold; color: #9ca3af; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 20px; border-bottom: 1px solid #f3f4f6; padding-bottom: 10px;">Customer Details</div>
                <p style="margin-bottom: 30px; color: #4b5563;">
                    <strong>Name:</strong> {{ $order->client_name ?? $order->user->name ?? 'N/A' }}<br>
                    <strong>Email:</strong> {{ $order->client_email ?? $order->user->email ?? 'N/A' }}<br>
                    <strong>Phone:</strong> {{ $order->client_phone ?? $order->user->phone ?? 'N/A' }}<br>
                ...
                <div style="background-color: #f9fafb; padding: 20px; border-radius: 15px; margin-top: 30px;">
                    ...
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 14px; margin-top: 10px;">
                        <span>Payment Method</span>
                        <span style="text-transform: uppercase; font-weight: bold;">{{ $order->payment_method ?? $order->payment->payment_method ?? 'N/A' }}</span>
                    </div>
                </div>
    
                <div style="text-align: center; margin-top: 20px;">
                    <a href="{{ route('admin.orders.index') }}" style="display: inline-block; background-color: #ea580c; color: white; padding: 15px 30px; text-decoration: none; border-radius: 12px; font-weight: bold; text-align: center;">Manage Order</a>
                </div>
            </div>
            <div style="text-align: center; padding: 30px; font-size: 12px; color: #9ca3af;">
                &copy; {{ date('Y') }} Bellion Bake & Brew Admin Panel. All rights reserved.
            </div>
        </div>
    </div>
</html>
